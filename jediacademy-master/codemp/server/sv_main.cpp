...
===============
*/
#define RCON_RATE_LIMIT_SLOTS 16
...
#define	SV_OUTPUTBUF_LENGTH	(MAX_MSGLEN - 16)
	...
	} rconRateLimit[RCON_RATE_LIMIT_SLOTS];
	int		bestSlot;

	time = Com_Milliseconds();
	bestSlot = -1;
	for ( i = 0; i < RCON_RATE_LIMIT_SLOTS; i++ ) {
		if ( rconRateLimit[i].lasttime == 0 ) {
			bestSlot = i;
			break;
		}
		if ( NET_CompareBaseAdr( from, rconRateLimit[i].addr ) ) {
			bestSlot = i;
			break;
		}
	}
	if ( bestSlot == -1 ) {
		...
		bestSlot = 0;
		for ( i = 1; i < RCON_RATE_LIMIT_SLOTS; i++ ) {
			if ( rconRateLimit[i].lasttime < rconRateLimit[bestSlot].lasttime ) {
				bestSlot = i;
			}
		}
	}
	if ( time < ( rconRateLimit[bestSlot].lasttime + 500 ) ) {
		return;
	}
	rconRateLimit[bestSlot].lasttime = time;
	rconRateLimit[bestSlot].addr = from;
	...
	if ( !valid ) {
		// Don't send any response on invalid rcon to prevent amplification/reflection attacks
		return;
	}
