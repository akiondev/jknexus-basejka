...
*/
	...
	if ( !NET_IsLocalAddress (from) ) {
		...
		Com_Printf( "%s", va(SE_GetString("MP_SVGAME", "CLIENT_CONN_WITH_PING"), i, ping) );//"Client %i connecting with %i challenge ping\n", i, ping );
...
void SV_DropClient( client_t *drop, const char *reason ) {
	...
	{
		char dropName[MAX_NAME_LENGTH];
		Q_strncpyz(dropName, drop->name, sizeof(dropName));
...
void SV_UserinfoChanged( client_t *cl ) {
	...
	// name for C code
	{
		char *val = Info_ValueForKey(cl->userinfo, "name");
		char cleanName[MAX_NAME_LENGTH];
		char *s, *d;
		int spaces = 0;

		for (s = val, d = cleanName; *s && (d - cleanName) < sizeof(cleanName) - 1; s++) {
			if (*s == '"' || *s == ';' || *s == '\n' || *s == '\r' || *s == '\\')
				continue;
			if (*s == ' ') {
				spaces++;
		...
		*d = 0;
		if (!cleanName[0])
			Q_strncpyz(cleanName, "Padawan", sizeof(cleanName));
